...
| ERR-011 | Blank white screen on production site for all visitors | Uncaught `ReferenceError: React is not defined` in JS bundle (`index-C8Yac8Ek.js:40:3471`) caused React to unmount silently due to missing `import React from "react"` default imports in `App.tsx` and page components | Added default `React` import (`import React, { ... } from "react"`) across `App.tsx` and all page components, cleaned duplicate `AuthGateway` import, and verified top-level `<ErrorBoundary>` fallback | Always include `import React from "react"` in `.tsx` files referencing `React.*` symbols or types, and wrap app root in top-level `ErrorBoundary` to prevent silent blank screens | 2026-08-23 |
| ERR-012 | Dashboard crash / ErrorBoundary recovery screen triggered for logged-in users | `ReferenceError: fetchCalendarBatches is not defined` in `Frontend/src/app/App.tsx:744` inside `DashOverview()` caused by missing `fetchCalendarBatches` in `./api` import list | Added `fetchCalendarBatches` to `./api` import list in `App.tsx` and verified clean typecheck/build | Always cross-reference exported helper calls against top-level module import lists when adding data fetching hooks | 2026-08-23 |
| ERR-013 | Unauthenticated `/login` route renders home landing page instead of login form; dead root Next.js app confusion risk | `/login` was missing from `pathMap` in `Frontend/src/app/App.tsx`, causing `getInitialPage()` to fall back to `"home"`; obsolete root Next.js app files created developer confusion | Added `"/login": "dashboard"` to `pathMap` in `App.tsx` so `/login` route renders `ProtectedRoute` -> `AuthGateway`; purged dead root Next.js app (`/app`, `middleware.ts`, `providers/AuthProvider.tsx`, etc.) | Ensure all client-side URL routes are explicitly mapped in SPA route lookup tables; purge un-deployed dead framework code to prevent maintenance drift | 2026-08-23 |

...
| ERR-008 | GET /api/v2/numbers/search returns 502 with generic "Unable to fetch available numbers" | Double slashes from trailing slashes in VOBIZ_API_URL or un-trimmed credentials cause Vobiz API rejection; route catch block swallowed real error with static generic message | Added .trim() and trailing slash stripping to VobizIntegrationService constructor; updated numbers.ts catch block to sanitize and surface real failure reason to client | Always sanitize URL and credential configuration strings at provider initialization; never swallow provider API error details in catch blocks | 2026-08-17 |

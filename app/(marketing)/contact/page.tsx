    ...
    <div className="space-y-16 py-16 px-6 max-w-7xl mx-auto">
      ...
      <div className="text-center space-y-4 max-w-3xl mx-auto">
        ...
        <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-white leading-tight font-sans">
          Schedule a Voice Design Demo
        </h1>
        <p className="text-slate-400 text-sm md:text-base leading-relaxed">
          Need a compliant voice agent configuration or dedicated model integration? Speak to our team of Voice AI integration leads.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
        ...
        <div className="md:col-span-1 space-y-6">
          <Card className="bg-slate-900/20 border-white/5 backdrop-blur">
          ...
          <Card className="bg-slate-900/20 border-white/5 backdrop-blur">
            ...
            <CardContent className="pt-4 space-y-4 text-xs text-slate-400">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white font-semibold font-sans">architects@clarityvoice.ai</span>
              </div>
              <div className="flex items-center gap-2">
                <Phone className="h-4 w-4 text-white" />
                <span className="text-white font-semibold font-sans">+1 (888) 555-VOIP</span>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="bg-slate-900/40 border border-white/5 rounded-xl p-5 space-y-3">
            ...
            <Link href="/test-hr" className="block pt-1">
              <Button size="sm" className="w-full bg-white hover:bg-slate-200 text-slate-950 font-bold text-xs h-8">
                Call My Phone Now
              </Button>
            </Link>
          </div>
        ...
        <div className="md:col-span-2">
          <Card className="bg-slate-900/20 border-white/5 backdrop-blur shadow-2xl relative overflow-hidden h-full flex flex-col justify-between">
            <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-3xl pointer-events-none" />
            
            <CardHeader className="pb-4">
              <CardTitle className="text-lg font-bold text-white">Request Consultation</CardTitle>
              <CardDescription className="text-slate-400 text-xs">
                Submit your query below. Our team will contact you in under 4 hours to coordinate custom layout integrations.
              </CardDescription>
            </CardHeader>
              ...
              {submitted ? (
                <div className="p-6 rounded-lg border border-emerald-950/30 bg-emerald-950/20 text-emerald-400 text-center space-y-2 max-w-md mx-auto my-6">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">
                    Thank you. A calendar link has been sent to your inbox to schedule a live session with our design leads.
                  </p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        className="bg-slate-950/80 border-white/5 text-xs h-10 text-white focus:border-white/10"
                        required
                        disabled={loading}
                      />
                    </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      placeholder="We want to explore custom voice agent outreach for our hiring campaigns..."
                      value={formData.message}
                      onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                      className="flex min-h-[100px] w-full rounded-md border border-white/5 bg-slate-950/80 px-3 py-2 text-xs text-white placeholder:text-slate-600 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-white/10 disabled:cursor-not-allowed disabled:opacity-50"
                      required
                      disabled={loading}
                    />
                  </div>
                  ...
                  <Button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-white hover:bg-slate-200 text-slate-950 font-bold h-10 text-xs shadow"
                  >
                    {loading ? (
                      <>

...
export default function DashboardLayout({
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-800">
      ...
      <aside
        className={cn(
          'bg-white border-r border-slate-250/80 transition-all duration-300 ease-in-out z-20 flex flex-col',
          isSidebarOpen ? 'w-[280px]' : 'w-[80px]'
        )}
      >
        {/* Sidebar Header with Brand */}
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-250/80">
          <div className="flex items-center gap-2.5 overflow-hidden">
            <div className="h-8 w-8 rounded-lg bg-emerald-600 flex items-center justify-center shrink-0">
              <Sparkles className="h-4.5 w-4.5 text-white" />
            </div>
            {isSidebarOpen && (
              <span className="font-semibold text-sm text-slate-800 tracking-tight font-sans whitespace-nowrap">
                Clarity AI Voice
              </span>
            )}
          </div>
          <button
            ...
            className="text-slate-500 hover:text-slate-700 focus:outline-none shrink-0"
          >
        ...
        {isSidebarOpen && activeUser?.tenant_name && (
          <div className="mx-4 my-3 p-3 bg-slate-100/60 rounded-lg border border-slate-200/50 flex flex-col space-y-1">
            <span className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">Workspace</span>
            <span className="text-xs font-semibold text-slate-700 truncate">{activeUser.tenant_name}</span>
          </div>
        )}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.label} href={item.href}>
                ...
                >
                  <item.icon className={cn('h-4 w-4 shrink-0 transition-transform duration-200 group-hover:scale-110', isActive ? 'text-emerald-600' : 'text-slate-500')} />
        ...
        {/* Documentation links */}
        <div className="p-3 border-t border-slate-250/80">
          <a
            href="https://docs.bolna.ai"
            target="_blank"
            rel="noopener noreferrer"
            className={cn(
              'flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100/50 transition-colors',
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 z-10 shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 w-64 text-slate-400 focus-within:border-slate-300 focus-within:text-slate-750 transition-all">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 bg-white">
          {/* Breadcrumbs and Page title */}
          <div className="flex items-center gap-2 text-xs font-medium text-slate-400">
            <span className="hover:text-slate-655 cursor-pointer">Platform</span>
            <ChevronRight className="h-3 w-3" />
            {activeUser?.tenant_name && (
              <>
                <span className="hover:text-slate-655 cursor-pointer max-w-[120px] truncate">{activeUser.tenant_name}</span>
                <ChevronRight className="h-3 w-3" />
              </>
            )}
